...
export default function NewCoverageAreaPage() {
  const t = useTranslations();
  const locale = useLocale();
  const router = useRouter();

  const [form, setForm] = useState<FormData>({
    partnerId: "", contractId: "", city: "", operationalZone: "", districts: "",
    coverageType: "", coverageDays: "", coverageStartTime: "", coverageEndTime: "",
    minExpectedShipments: "", maxExpectedShipments: "",
    needsDedicatedDrivers: false, status: "draft",
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error("Failed to create coverage area");
      const data = await res.json();
      const id = data.coverageArea?._id ?? data._id;
  ...
  return (
    <DashboardLayout locale={locale} title={t("coverageAreas.new")}>
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-gray-900">{t("coverageAreas.new")}</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex items-center justify-end gap-3">
            <Button type="button" variant="outline" onClick={() => router.push("/coverage-areas")}>
              {t("common.cancel")}
            </Button>
