...
import { Search, Plus, Eye, Pencil, Truck, AlertCircle, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function VehiclesPage() {
  ...
  return (
    <DashboardLayout locale={locale} title={t("vehicles.title")}>
      <div className="space-y-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-gray-900">{t("vehicles.title")}</h1>
          ...
          <CardContent>
            {loading ? (
              ...
              <EmptyState
                ...
                action={{ label: t("errors.retry"), onClick: fetchVehicles }}
              />
            ) : vehicles.length === 0 ? (
              <EmptyState
                icon={<Truck className="size-8" />}
                title={t("common.noData")}
                action={{ label: t("vehicles.new"), onClick: () => router.push("/vehicles/new") }}
              />
            ) : (
              <>
                    ...
                    <TableBody>
                      {vehicles.map((vehicle) => (
                        <TableRow key={vehicle._id}>
                          ...
                          <TableCell>{vehicle.assignedDriver?.fullName ?? "-"}</TableCell>
                          <TableCell>
