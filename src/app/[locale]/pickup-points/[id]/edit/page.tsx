...
export default function EditPickupPointPage() {
  ...
  const fetchPoint = useCallback(async () => {
    ...
    try {
      ...
      const point = data.pickupPoint ?? data;
      setForm({
        partnerId: point.partner?._id ?? "",
        name: point.name ?? "",
        pointType: point.pointType ?? "",
        city: point.city ?? "",
        district: point.district ?? "",
        address: point.address ?? "",
        nationalAddress: point.nationalAddress ?? "",
        ...
        requiresAppointment: point.requiresAppointment ?? false,
  ...
  if (loading) {
    return (
      <DashboardLayout locale={locale} title={t("pickupPoints.edit")}>
    ...
    <DashboardLayout locale={locale} title={t("pickupPoints.edit")}>
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="icon" onClick={() => router.push(`/pickup-points/${id}`)}>
            <ArrowLeft className="size-4" />
          </Button>
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-gray-900">{t("pickupPoints.edit")}</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex items-center justify-end gap-3">
            <Button type="button" variant="outline" onClick={() => router.push(`/pickup-points/${id}`)}>
              {t("common.cancel")}
            </Button>
